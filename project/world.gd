...
func place_block(coords: Vector2i, tile_atlas_coords: Vector2i) -> void:
	if tile_map.get_cell_source_id(coords) == -1:
		tile_map.set_cell(coords, SOURCE_ID, tile_atlas_coords)
		if block_damage_map.has(coords):
			block_damage_map.erase(coords)
...
func generate_chunk(chunk_x: int) -> void:
	...
	for x in range(CHUNK_WIDTH):
		...
		for y in range(surface_y, surface_y + CHUNK_HEIGHT):
			...
			# Back walls only underground (4 blocks below surface)
